...
namespace BanGiay.Form.US
{
    public partial class Frm_US_BanHang : UserControl
    {
        ...
        private void LoadHinhThucThanhToan()
        {
            ...
            // Kiểm tra danh sách có phần tử nào hay không trước khi gán DataSource
            if (_lstHinhThucThanhToan != null && _lstHinhThucThanhToan.Any())
            {
                cbbHinhThucThanhToan.DataSource = _lstHinhThucThanhToan.ToList();
                cbbHinhThucThanhToan.DisplayMember = "TENHINHTHUC";
                cbbHinhThucThanhToan.ValueMember = "MAHINHTHUCTHANHTOAN";
                cbbHinhThucThanhToan.SelectedIndex = 0; // Chỉ đặt SelectedIndex nếu danh sách không rỗng
            }
            else
            {
                // Xử lý khi danh sách rỗng hoặc null, ví dụ:
                cbbHinhThucThanhToan.DataSource = null;
                cbbHinhThucThanhToan.Items.Clear(); // Xóa tất cả mục hiện tại
            }
        }
        ...
        private void ptbThemHoaDon_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (confirmResult == DialogResult.OK)
                {
                    // Kiểm tra objUuDai trước khi sử dụng
                    var mauUuDai = objUuDai?.Mauudai ?? 1; // Nếu objUuDai là null, sử dụng giá trị mặc định là 1
                    ...
                    var result = _Ser_HoaDon.Them(new Hoadon()
                    {
                        ...
                        Sdtcuahang = "",
                        Emailcuahang = "",
                        Ngaytao = DateTime.Now, // Sửa lại nếu bạn muốn lấy DateTime hiện tại
                        Tongtien = null,
                        Ghichu = "",
                        Trangthai = false
                    });
        ...
        private void LoadMaGiamGia()
        {
            ...
            if (objUuDai == null || objUuDai.Soluong <= 0)
            {
                txtGiamGia.Text = "Đã hết ưu đãi";
            }
            else
            {
                txtGiamGia.Text = $"{objUuDai.Phantram} % - {objUuDai.Tenuudai}";
            }
        }
